...
type Mastery={attempts:number;bestScore:number;lastScore:number;lastPracticed:string;status:"new"|"learning"|"review"|"mastered"};
const STORE="speakflow:pronunciation-mastery";
function masteryLabel(s:Mastery["status"]){return s==="mastered"?"Dominado":s==="review"?"Revisar":s==="learning"?"Aprendendo":"Novo"}
const drills:Drill[]=[
...
export default function PronunciationPage(){
 const router=useRouter();const [insights,setInsights]=useState<Insight[]>([]);const [loading,setLoading]=useState(true);const [index,setIndex]=useState(0);const [listening,setListening]=useState(false);const [speaking,setSpeaking]=useState(false);const [result,setResult]=useState<{heard:string;score:number}|null>(null);const [mastery,setMastery]=useState<Record<string,Mastery>>({});
 useEffect(()=>{try{setMastery(JSON.parse(localStorage.getItem(STORE)||"{}"))}catch{};(async()=>{const {data:{session}}=await supabase.auth.getSession();if(!session){router.replace("/login");return}const {data,error}=await supabase.rpc("get_lab_insights",{p_lab:"pronunciation"});if(error){console.error("Erro ao carregar pronunciation lab:",error);setLoading(false);return;}setInsights((data||[]).filter(x=>isPronunciation(x.skill_category)));setLoading(false)})()},[router]);
 function saveAttempt(target:string,score:number){const key=target.toLowerCase();const old=mastery[key];const attempts=(old?.attempts||0)+1;const bestScore=Math.max(old?.bestScore||0,score);const status:Mastery["status"]=bestScore>=90&&attempts>=3?"mastered":bestScore>=75?"review":"learning";const next={...mastery,[key]:{attempts,bestScore,lastScore:score,lastPracticed:new Date().toISOString(),status}};setMastery(next);localStorage.setItem(STORE,JSON.stringify(next))}
