...
import { ArrowLeft, ArrowRight, Check, LockKeyhole, Crown, Gem, BadgeCheck } from "lucide-react";
  ...
  return <main className="profile-shell">
    <section className="profile-card">
      ...
      <div className="profile-hero">
        ...
        <div><span>{hasActivePro ? "MEMBRO SPEAKFLOW PRO" : "PERFIL DO ALUNO"}</span><div className="profile-name-row"><h1>{name}</h1>{hasActivePro && <span className="profile-pro-badge"><Crown aria-hidden="true" /> PRO</span>}</div><p>{email}</p>{hasActivePro && <div className="profile-premium-status"><div className="profile-premium-glow" aria-hidden="true" /><span className="profile-premium-mark"><Gem aria-hidden="true" /></span><div><small>EXPERIÊNCIA PREMIUM ATIVA</small><strong>Seu SpeakFlow está completo.</strong><p>Coach avançado, Labs completos e aprendizado adaptativo liberados para a sua jornada.</p></div><span className="profile-premium-verified"><BadgeCheck aria-hidden="true" /> PRO ATIVO</span></div>}
