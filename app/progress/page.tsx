...
export default function ProgressPage(){
 ...
 const modeSummary=useMemo(()=>{const summary={conversation:{sessions:0,seconds:0},vocabulary:{sessions:0,seconds:0},pronunciation:{sessions:0,seconds:0}};sessions.forEach(x=>{const key=x.mode==="vocabulary"?"vocabulary":x.mode==="pronunciation"?"pronunciation":x.mode==="conversation"?"conversation":null;if(!key)return;summary[key].sessions+=1;summary[key].seconds+=Math.max(0,x.duration_seconds??0)});return summary},[sessions]);
  ...
  <section className="progress-main">
   ...
   <div className="progress-content">
    ...
    {loading?<section className="progress-state">Carregando sua jornada...</section>:<>
     ...
     {smartSummary&&<section className="progress-card progress-smart"><div className="progress-card-heading"><div><span>SPEAKFLOW INTELLIGENCE</span><h2>Resumo inteligente</h2></div><small>Baseado no seu histórico real</small></div><div className="smart-summary-grid"><div className="smart-summary-main"><span>LEITURA ATUAL</span><h3>{smartSummary.headline}</h3><p>Você concluiu {smartSummary.recentSessions} {smartSummary.recentSessions===1?"sessão":"sessões"} nos últimos 7 dias{smartSummary.topSkill?`, e ${smartSummary.topSkill.toLowerCase()} aparece como seu foco mais recorrente.`:"."}</p></div><div className="smart-summary-next"><span>PRÓXIMO FOCO</span><p>{smartSummary.recommendation}</p>{smartSummary.latestTip&&<small>✦ Insight recente: {smartSummary.latestTip}</small>}</div></div></section>}
     ...
     <section className="progress-card progress-sessions"><div className="progress-card-heading"><div><span>SUA JORNADA</span><h2>Histórico inteligente</h2></div><small>Resumo das sessões concluídas</small></div>{sessions.length?<div className="session-history">{sessions.slice(0,8).map((x,n)=><article className={x.summary?"session-history-card has-summary":"session-history-card"} key={x.id??`${x.created_at}-${n}`}><div className="session-history-top"><div><span className="session-icon">✦</span><div><strong>{x.mode==="conversation"?"Conversação":x.mode==="vocabulary"?"Vocabulário":x.mode==="pronunciation"?"Pronúncia":x.mode||"Prática"}</strong><small>{formatDate(x.created_at)} · {formatDuration(x.duration_seconds)}</small></div></div>{x.skills_practiced&&<span className="session-skill">{x.skills_practiced}</span>}</div>{x.summary?<><p className="session-history-summary">{x.summary}</p><div className="session-history-details">{x.positive_point&&<div><span>PONTO POSITIVO</span><p>{x.positive_point}</p></div>}{x.improvement_point&&<div><span>PRÓXIMA EVOLUÇÃO</span><p>{x.improvement_point}</p></div>}</div>{x.next_recommendation&&<div className="session-history-next"><span>✦ RECOMENDAÇÃO DO COACH</span><p>{x.next_recommendation}</p></div>}</>:<p className="session-history-legacy">Sessão registrada antes da introdução dos resumos inteligentes.</p>}</article>)}</div>:<p className="progress-empty">Sua primeira sessão concluída aparecerá aqui.</p>}</section>
    </>}
   </div>
  </section>
 </main>
