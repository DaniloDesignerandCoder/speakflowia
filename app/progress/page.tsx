...
export default function ProgressPage(){
 ...
 const skills=useMemo(()=>{const normalizeSkill=(value:string|null)=>{const raw=value?.trim().toLowerCase()||"";if(!raw)return "Outros";if(raw.includes("pronun")||raw.includes("pronunciation"))return "Pronúncia";if(raw.includes("gram")||raw.includes("grammar"))return "Gramática";if(raw.includes("vocab")||raw.includes("word choice"))return "Vocabulário";if(raw.includes("flu")||raw.includes("fluency"))return "Fluência";if(raw.includes("listen")||raw.includes("compreens"))return "Compreensão";if(raw.includes("speak")||raw.includes("conversation")||raw.includes("conversa"))return "Conversação";return "Outros"};const m=new Map<string,number>();insights.forEach(x=>{const c=normalizeSkill(x.skill_category);m.set(c,(m.get(c)??0)+1)});return Array.from(m.entries()).map(([name,count])=>({name,count})).sort((a,b)=>b.count-a.count)},[insights]);
 const maxSkill=Math.max(1,...skills.map(x=>x.count));
 const skillEvolution=useMemo(()=>{const now=Date.now();const recentCutoff=now-7*24*60*60*1000;const previousCutoff=now-14*24*60*60*1000;const normalize=(value:string|null)=>{const raw=value?.trim().toLowerCase()||"";if(!raw)return "Outros";if(raw.includes("pronun")||raw.includes("pronunciation"))return "Pronúncia";if(raw.includes("gram")||raw.includes("grammar"))return "Gramática";if(raw.includes("vocab")||raw.includes("word choice"))return "Vocabulário";if(raw.includes("flu")||raw.includes("fluency"))return "Fluência";if(raw.includes("listen")||raw.includes("compreens"))return "Compreensão";if(raw.includes("speak")||raw.includes("conversation")||raw.includes("conversa"))return "Conversação";return "Outros"};const map=new Map<string,{recent:number;previous:number}>();insights.forEach(x=>{const time=new Date(x.created_at).getTime();const name=normalize(x.skill_category);const entry=map.get(name)??{recent:0,previous:0};if(time>=recentCutoff)entry.recent+=1;else if(time>=previousCutoff)entry.previous+=1;map.set(name,entry)});return map},[insights]);
