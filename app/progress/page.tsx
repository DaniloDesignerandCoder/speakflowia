...
export default function ProgressPage(){
 ...
 const modeSummary=useMemo(()=>{const summary={conversation:{sessions:0,seconds:0},vocabulary:{sessions:0,seconds:0}};sessions.forEach(x=>{const key=x.mode==="vocabulary"?"vocabulary":x.mode==="conversation"?"conversation":null;if(!key)return;summary[key].sessions+=1;summary[key].seconds+=Math.max(0,x.duration_seconds??0)});return summary},[sessions]);
